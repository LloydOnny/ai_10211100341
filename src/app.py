...
Streamlit UI: hybrid retrieval, query expansion, optional conversation memory,
prompt templates, evaluation baselines, pipeline logs, LLM answer.
"""
    ...
    if api_key:
        ...
        with st.spinner("Generating answer with LLM..."):
            ...
            try:
                ...
                with st.expander("Evaluation: baseline, consistency, grounding", expanded=False):
                    st.caption(
                        "Runs additional API calls: same RAG prompt twice at temperature 0 (consistency check), "
                        "a general-knowledge baseline without your documents, and a short grounding review of the RAG answer."
                    )
                    if st.button("Run evaluation for this query"):
                        with st.spinner("Running evaluation..."):
                            bundle = run_evaluation_bundle(chunks, query, answer, prompt, api_key)
                        st.markdown("### E.3 — RAG vs pure LLM (same question)")
                        st.caption(f"Question: {query}")
                        rag_col, pure_col = st.columns(2)
                        with rag_col:
                            st.markdown("**With retrieval (RAG)**")
                            st.write(answer)
                        with pure_col:
                            st.markdown("**General knowledge only (pure LLM)**")
                            st.write(bundle["pure_llm_answer"])
                        st.markdown("---")
                        st.markdown("**Consistency (same RAG prompt, temperature 0 twice)**")
                        st.write(bundle["consistency"])
                        st.markdown("**Grounding judge (first line = label)**")
                        st.code(bundle.get("grounding_label_line", ""), language="text")
                        st.text(bundle.get("grounding_judge_raw", ""))

                if use_memory:
                    st.session_state["chat_history"].append({"role": "user", "content": query})
                    st.session_state["chat_history"].append({"role": "assistant", "content": answer})
                    while len(st.session_state["chat_history"]) > 12:
                        st.session_state["chat_history"].pop(0)

            except Exception as e:
                st.error(f"LLM error: {e}")
    else:
        st.info("Enter your OpenAI API key in the sidebar to generate an answer.")
